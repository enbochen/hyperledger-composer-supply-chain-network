...
/**
 * Track the trade of a commodity from one trader to another
 * @param {org.hcsc.network.AssembleParts} assembleParts - the trade to be processed
 * @transaction
 */
function assembleParts (assembleParts) {
    console.log(assembleParts);
    
    var NS = 'org.hcsc.network';
    var factory = getFactory();
    var me = getCurrentParticipant();
    
    var newCommodity = factory.newResource(NS, 'Commodity', assembleParts.tradingSymbol);
    newCommodity.name = assembleParts.name;
    newCommodity.description = assembleParts.description;
    newCommodity.quantity = assembleParts.quantity;
    newCommodity.unitPrice = assembleParts.unitPrice;
    newCommodity.totalPrice = assembleParts.totalPrice;
    newCommodity.trace= [];
    newCommodity.trace.children = [];
    newCommodity.children = [];
    newCommodity.owner = me;
    
    assembleParts.children.forEach(function (child) {
        if (child.trace.length > 0 ) {
            child.trace.forEach(function(trace) {
                newCommodity.trace.push(trace);
            })
        }
        newCommodity.children.push(child);
    })

   	return getAssetRegistry(newCommodity.getFullyQualifiedType())
   		.then(function (assetRegistry) {
       		return assetRegistry.add(newCommodity);
     	});
 }
